...
import PRGAExcuteThread from './src/utils/PRGAThread'
...
class Rc4Md5 {
  ...
  constructor(password, sizeSalt) {
    ...
    if (!sizeSalt) {
      // 用于测试
      this.fileHexKey = crypto.createHash('md5').update(password).digest('hex')
      return this.resetKSA()
    }
    ...
    // passwdOutward: file passwd，could be share safe
  ...
  resetKSA() {
    ...
    // 首次加密rc4Key = fileHexKey
    this.initKSA(rc4Key.toString('hex'))
  }
  ...
  _nextByte() {
    ...
    const t = (this.sbox[this.i] + this.sbox[this.j]) & 0xff
    return this.sbox[t]
  }
  ...
  initKSA(key) {
    ...
    for (let i = 0, j = 0; i < 256; i++) {
      j = (j + this.sbox[i] + key[i % key.length]) & 0xff;
...
// 测试
const plainText = "Hello RC4 流密码测试"
const rc4 = new Rc4Md5('1234567')
const buffer = rc4.encryptText(plainText)

console.log('@@@密文', buffer)
// 要记得重置
rc4.setPosition(0)
const plainBy = rc4.encrypt(buffer)
